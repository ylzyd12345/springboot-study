...
public class UserController {
    ...
    public ResponseEntity<ApiResponse<User>> createUser(
        ...
        try {
            ...
            log.error("用户已存在: username={}", request.getUsername(), e);
            return ResponseEntity.ok(ApiResponse.error(ErrorCode.USER_ALREADY_EXISTS.getCode(), "用户已存在"));
        } catch (DataAccessException e) {
            log.error("数据库操作失败: username={}", request.getUsername(), e);
            return ResponseEntity.ok(ApiResponse.error("系统繁忙，请稍后重试"));
        } catch (BusinessException e) {
            log.error("业务异常: username={}, error={}", request.getUsername(), e.getMessage(), e);
    ...
    public ResponseEntity<ApiResponse<Page<User>>> getUsers(
        ...
        try {
            ...
            sample.stop(Timer.builder("pagination.query.duration")
                    ...
                    .tag("status", "error")
                    .register(meterRegistry));
            log.error("查询参数错误: current={}, size={}", request.getCurrent(), request.getSize(), e);
            return ResponseEntity.ok(ApiResponse.error("查询参数错误"));
        } catch (DataAccessException e) {
            sample.stop(Timer.builder("pagination.query.duration")
                    .tag("entity", "user")
                    .tag("status", "error")
    ...
    public ResponseEntity<ApiResponse<User>> updateUser(
        ...
        try {
            ...
            log.error("邮箱已存在: id={}", id, e);
            return ResponseEntity.ok(ApiResponse.error(ErrorCode.USER_ALREADY_EXISTS.getCode(), "邮箱已存在"));
        } catch (DataAccessException e) {
            log.error("数据库操作失败: id={}", id, e);
            return ResponseEntity.ok(ApiResponse.error("系统繁忙，请稍后重试"));
        } catch (BusinessException e) {
            log.error("业务异常: id={}, error={}", id, e.getMessage(), e);
            return ResponseEntity.ok(ApiResponse.error(e.getMessage()));
        } catch (Exception e) {
    ...
    public ResponseEntity<ApiResponse<Void>> batchDeleteUsers(
        ...
        try {
            ...
            log.error("数据库操作失败: ids={}", ids, e);
            return ResponseEntity.ok(ApiResponse.error("系统繁忙，请稍后重试"));
        } catch (BusinessException e) {
            log.error("业务异常: ids={}, error={}", ids, e.getMessage(), e);
            return ResponseEntity.ok(ApiResponse.error(e.getMessage()));
        } catch (Exception e) {
