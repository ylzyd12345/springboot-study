...
class RedliningValidator:
    ...
    def validate(self):
        ...
        # First, check if there are any tracked changes by iFlow CLI to validate
        try:
            ...
            # Check for w:del or w:ins tags authored by iFlow CLI
            del_elements = root.findall(".//w:del", self.namespaces)
            ins_elements = root.findall(".//w:ins", self.namespaces)

            # Filter to only include changes by iFlow CLI
            claude_del_elements = [
                elem
                for elem in del_elements
                if elem.get(f"{{{self.namespaces['w']}}}author") == "iFlow CLI"
            ]
            claude_ins_elements = [
                elem
            ...
            # Redlining validation is only needed if tracked changes by iFlow CLI have been used.
            if not claude_del_elements and not claude_ins_elements:
                if self.verbose:
                    print("PASSED - No tracked changes by iFlow CLI found.")
                return True
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Remove iFlow CLI's tracked changes from both documents
            ...
            if self.verbose:
                print("PASSED - All changes by iFlow CLI are properly tracked")
            return True

    def _generate_detailed_diff(self, original_text, modified_text):
        """Generate detailed word-level differences using git word diff."""
        error_parts = [
            "FAILED - Document text doesn't match after removing iFlow CLI's tracked changes",
            "",
            "Likely causes:",
    ...
    def _remove_claude_tracked_changes(self, root):
        """Remove tracked changes authored by iFlow CLI from the XML root."""
        ...
        for parent in root.iter():
            to_remove = []
            for child in parent:
                if child.tag == ins_tag and child.get(author_attr) == "iFlow CLI":
                    to_remove.append(child)
            for elem in to_remove:
                parent.remove(elem)

        # Unwrap content in w:del elements where author is "iFlow CLI"
        ...
        for parent in root.iter():
            to_process = []
            for child in parent:
                if child.tag == del_tag and child.get(author_attr) == "iFlow CLI":
