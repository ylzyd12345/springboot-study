...
# OpenSpec Agent for iFlow CLI

此智能体为iFlow CLI提供完整的OpenSpec规范驱动开发功能。
...
/openspec show [change-id]        # 显示变更详情
/openspec validate [item]         # 验证规范
...
node bin/openspec.js show [item]
node bin/openspec.js validate [item] --strict

# 项目管理  
node bin/openspec.js init [path]
node bin/openspec.js update [path]
```
