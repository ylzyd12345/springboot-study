# Junmo Platform 技术架构设计文档

## 📋 文档基本信息

| 项目名称 | Junmo Platform 企业级智能管理平台 |
|---------|------------------------------|
| **文档名称** | Junmo Platform 技术架构设计 |
| **版本号** | v1.3.0 |
| **创建日期** | 2025-12-24 |
| **更新日期** | 2026-01-11 |
| **作者** | 技术架构师 |
...
**v1.3.0 (2026-01-11)**:
- 消息队列从 Spring Cloud Stream 改为 Spring Boot 原生集成
- 使用 Spring Boot AMQP (RabbitTemplate) 替代 Spring Stream RabbitMQ Binder
- 使用 Spring Kafka (KafkaTemplate) 替代 Spring Stream Kafka Binder
- 完全移除对 Spring Cloud 生态的依赖
- 更新消息队列实现示例和配置
...
graph TB
    ...
    subgraph "Service层 (Service Layer)"
        C1[业务服务] --> C2[Business Services]
        C3[事务管理] --> C4[Transaction Management]
        C5[业务规则] --> C6[Business Rules]
        C7[工作流] --> C8[Workflow Engine]
    end

    subgraph "Mapper层 (Mapper Layer)"
        D1[数据访问] --> D2[MyBatis-Plus]
        D3[SQL执行] --> D4[SQL Execution]
        D5[结果映射] --> D6[Result Mapping]
    end

    subgraph "基础设施层 (Infrastructure Layer)"
        E1[消息中间件] --> E2[Spring Stream]
        E3[缓存服务] --> E4[Redis]
        E5[搜索引擎] --> E6[Elasticsearch]
    end
...
public class RateLimiterConfig {
    ...
    public LoadingCache<String, RateLimiter> rateLimiterCache() {
        return Caffeine.newBuilder()
                ...
                .build(key -> RateLimiter.create(100)); // 每秒100个请求
    }
...
#### ADR-001: 选择单体Spring Boot架构
**状态**: 已接受
**决策**: 使用单体Spring Boot应用架构，而非微服务架构
**理由**:
- 当前业务规模适合单体架构
- 开发团队规模较小，单体架构更易维护
- 降低系统复杂度和运维成本
...
#### ADR-004: 使用Guava限流替代Sentinel
**状态**: 已接受
**决策**: 使用Guava RateLimiter进行限流，而非Sentinel
**理由**:
- 单体应用不需要分布式限流
- Guava轻量级，集成简单
- 性能开销小，适合单体应用场景
...
- 正面: 轻量级、高性能、易集成
- 负面: 不支持分布式限流，功能相对简单
- 风险: 随着应用扩展可能需要升级到分布式限流方案

#### ADR-005: 使用Spring Boot原生API接入消息队列
**状态**: 已接受
**决策**: 使用Spring Boot原生API（RabbitTemplate和KafkaTemplate）接入RabbitMQ和Kafka
**理由**:
- 更轻量级，无额外依赖
- 更直接的API，更易调试
- 更好的性能和资源利用
