# Junmo Platform 业务架构设计

## 📋 文档信息

| 项目 | 内容 |
|------|------|
| **文档名称** | Junmo Platform 业务架构设计 |
| **版本号** | v1.1.0 |
| **创建日期** | 2025-12-24 |
| **更新日期** | 2026-01-07 |
| **作者** | 业务架构师 |
...
**v1.1.0 (2026-01-07)**:
- 业务架构从DDD理念调整为三层架构理念
- 去掉限界上下文、聚合根、值对象、领域事件等DDD概念
- 更新包结构，采用entity、service、mapper三层结构
- 主键策略从IDENTITY调整为雪花算法

## 🎯 业务架构概述

Junmo Platform企业级智能管理平台的业务架构基于三层架构理念，采用清晰的分层结构划分业务领域，通过Service层保证业务一致性，以业务规则驱动业务流程。整个架构支持从单体向微服务的平滑演进。
...
```java
// 用户权限模块的包结构
com.junmo.Junmo Platform.core.user
├── entity/              // 实体类
│   ├── User.java
│   ├── Role.java
│   └── Permission.java
...
public class FileManagementDomainService {
    ...
    private String generateFilePath(String category, String originalFilename) {
        ...
        String uniqueId = IdWorker.getIdStr(); // 使用雪花算法生成唯一ID

        return String.format("%s/%s/%s%s", category, datePath, uniqueId, fileExtension);
    }
...
public class AuthenticationService {
    ...
    private String generateVerificationToken(Long userId) {
        String token = IdWorker.getIdStr(); // 使用雪花算法生成唯一ID
...
<definitions xmlns="http://www.omg.org/spec/BPMN/20100524/MODEL"
             targetNamespace="http://Junmo Platform.com/bpmn">
...
```drools
// 用户注册规则.drl
package com.junmo.Junmo Platform.rules

import com.junmo.Junmo Platform.core.domain.user.command.CreateUserCommand
import com.junmo.Junmo Platform.core.domain.user.validation.ValidationResult
