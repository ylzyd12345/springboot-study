# Junmo Platform 数据库详细设计文档

## 📋 文档信息

| 项目 | 内容 |
|------|------|
| **文档名称** | Junmo Platform 数据库详细设计 |
| **版本号** | v1.1.0 |
| **创建日期** | 2025-12-24 |
| **更新日期** | 2026-01-07 |
| **作者** | 数据库架构师 |
...
### 数据库架构目标
Junmo Platform采用多数据库架构，结合关系型数据库、文档数据库、缓存数据库和搜索引擎，为不同业务场景提供最优的数据存储解决方案。
...
graph TB
    subgraph "应用层"
        A1[Spring Boot应用] --> A2[业务逻辑]
        A3[缓存层] --> A4[Redis集群]
    end
    
    subgraph "数据访问层"
        B1[MyBatis-Plus] --> B2[MySQL主从]
        B3[MongoDB Driver] --> B4[MongoDB副本集]
        B5[ES Rest Client] --> B6[ES集群]
    end
...
spring:
  datasource:
    master:
      url: jdbc:mysql://mysql-master:3306/Junmo Platform?useUnicode=true&characterEncoding=utf8&useSSL=false&serverTimezone=Asia/Shanghai
...
spring:
  datasource:
    slave:
      url: jdbc:mysql://mysql-slave:3306/Junmo Platform?useUnicode=true&characterEncoding=utf8&useSSL=false&serverTimezone=Asia/Shanghai
      username: ${DB_USERNAME}
      password: ${DB_PASSWORD}
      driver-class-name: com.mysql.cj.jdbc.Driver
      hikari:
...
```yaml
# 缓存键命名规范
Junmo Platform:{module}:{type}:{id}:{field?}

# 示例
Junmo Platform:user:info:123              # 用户信息缓存
Junmo Platform:user:permissions:123       # 用户权限缓存
Junmo Platform:document:content:456       # 文档内容缓存
Junmo Platform:category:tree:root         # 分类树缓存
Junmo Platform:config:system:all          # 系统配置缓存
```
...
public class TokenCacheService {
    ...
    private static final String TOKEN_PREFIX = "Junmo Platform:token:";
...
relay-log=relay-bin
read-only=1
replicate-do-db=Junmo Platform
```
...
public class CacheSyncService {
    ...
    public void handleUserUpdatedEvent(UserUpdatedEvent event) {
        // 清除相关缓存
        String pattern = "Junmo Platform:user:*:" + event.getUserId();
        Set<String> keys = redisTemplate.keys(pattern);
        if (!keys.isEmpty()) {
            redisTemplate.delete(keys);
        }
    ...
    private void warmUpUserCache(Long userId) {
        ...
        redisTemplate.opsForValue().set(
            "Junmo Platform:user:info:" + userId, 
            user, 
...
spring:
  datasource:
    url: jdbc:mysql://mysql-master:3306/Junmo Platform?useSSL=true&requireSSL=true&verifyServerCertificate=false
    username: ${DB_USERNAME}
    password: ${DB_PASSWORD}
    driver-class-name: com.mysql.cj.jdbc.Driver
    hikari:
...
# 上传到云存储
aws s3 cp $BACKUP_DIR/full_backup_$DATE.sql.gz s3://Junmo Platform-backups/mysql/
```
...
# 上传到云存储
aws s3 cp $BACKUP_DIR/mongodb_backup_$DATE.tar.gz s3://Junmo Platform-backups/mongodb/
```
...
WHERE ACTION IN ('INSERT', 'UPDATE', 'DELETE')
  AND OBJECT_SCHEMA = 'Junmo Platform'
