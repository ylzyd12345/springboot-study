...
```bash
# 克隆项目仓库
git clone https://github.com/your-org/spring4demo.git
...
└── src/
    ...
    │   │       ├── Spring4demoApplication.java    # 主启动类
    │   │       ├── config/                        # 配置类
...
```java
// 正确示例
com.kev1n.spring4demo.controller
com.kev1n.spring4demo.service.impl
com.kev1n.spring4demo.repository

// 错误示例
com.Kev1n.Spring4demo.Controller
com.kev1n.spring4demo.controller.user
```
...
# 使用Java运行jar包
java -jar target/spring4demo-1.0.0.jar

# 指定配置文件运行
java -jar target/spring4demo-1.0.0.jar --spring.profiles.active=dev

# 指定JVM参数运行
java -Xms512m -Xmx1024m -jar target/spring4demo-1.0.0.jar
```
...
```bash
# 构建镜像
docker build -t spring4demo:1.0.0 .

# 构建镜像并指定标签
docker build -t spring4demo:latest -t spring4demo:1.0.0 .

# 查看镜像
docker images | grep spring4demo
```
...
# 查看容器日志
docker logs -f spring4demo

# 进入容器
docker exec -it spring4demo bash
```
