...
```bash
# 构建生产镜像
docker build -t spring4demo:{version} .
docker tag spring4demo:{version} spring4demo:latest

# 推送到镜像仓库
docker push spring4demo:{version}
docker push spring4demo:latest
...
```bash
# 检查应用状态
curl -f https://api.spring4demo.com/actuator/health
...
```bash
# 查看应用日志
kubectl logs -f deployment/spring4demo -n production

# 检查错误日志
kubectl logs deployment/spring4demo -n production | grep ERROR
...
```bash
# 查看当前版本
kubectl get deployment spring4demo -o yaml | grep image

# 回滚到上一个版本
kubectl rollout undo deployment/spring4demo -n production

# 回滚到指定版本
kubectl rollout undo deployment/spring4demo --to-revision=2 -n production

# 验证回滚
kubectl rollout status deployment/spring4demo -n production
```
...
# 重启服务
kubectl rollout restart deployment/spring4demo -n production
```
...
#### 应急联系方式
- **技术支持热线**: 400-123-4567
- **紧急联系邮箱**: emergency@spring4demo.com
- **应急响应群**: 企业微信群/钉钉群
