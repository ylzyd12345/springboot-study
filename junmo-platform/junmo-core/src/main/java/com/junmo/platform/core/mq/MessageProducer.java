        ...
        try {
            ...
            future.whenComplete((result, ex) -> {
                if (ex == null) {
                    ...
                    throw new com.junmo.platform.common.exception.BusinessException("发送用户创建消息到Kafka失败", ex);
                }
            });
            ...
            throw new com.junmo.platform.common.exception.BusinessException("Kafka序列化失败", e);
        } catch (org.apache.kafka.common.errors.TimeoutException e) {
            log.error("Kafka发送超时，无法发送用户创建消息: userId={}, error={}", userId, e.getMessage());
            throw new com.junmo.platform.common.exception.BusinessException("Kafka发送超时", e);
        } catch (org.apache.kafka.common.KafkaException e) {
            log.error("Kafka发送用户创建消息失败: userId={}, error={}", userId, e.getMessage());
            throw new com.junmo.platform.common.exception.BusinessException("Kafka发送失败", e);
        } catch (IllegalArgumentException e) {
            log.error("用户创建消息参数验证失败: userId={}, error={}", userId, e.getMessage());
            throw e;
        } catch (NullPointerException e) {
        ...
        try {
            ...
            future.whenComplete((result, ex) -> {
                if (ex == null) {
                    ...
                    throw new com.junmo.platform.common.exception.BusinessException("发送通知消息到Kafka失败", ex);
                }
            });
        } catch (org.apache.kafka.common.errors.SerializationException e) {
